...
if (!class_exists("EFS")) {
  class EFS
  {
    ...
    public function __construct()
    {
      ...
      add_action("after_setup_theme", ["EFS", "setup"]);
      add_action("init", ["EFS", "setup"]);
      add_action("switch_theme", ["EFS", "setup"]);
      add_action("admin_enqueue_scripts", ["EFS", "add_admin_enqueue_scripts"]);
      add_action(
        "wp_enqueue_scripts",
        ["EFS", "add_typography_enqueue_styles"],
        80
      );
      add_action("wp_head", ["EFS", "add_custom_css"], 80);
      add_filter("admin_body_class", ["EFS", "add_admin_body_class"]);
    }
    ...
    public static function setup()
    {
      ...
      if (class_exists("EFS_Options") && !empty(self::$args["admin_options"])) {
        foreach (self::$args["admin_options"] as $key => $value) {
          if (
            !empty(self::$args["sections"][$key]) &&
            !isset(self::$inited[$key])
          ) {
            $params["args"] = $value;
      ...
      $params = [];
      if (
        class_exists("EFS_Customize_Options") &&
        !empty(self::$args["customize_options"])
      ) {
      ...
      $params = [];
      if (
        class_exists("EFS_Metabox") &&
        !empty(self::$args["metabox_options"])
      ) {
      ...
      $params = [];
      if (
        class_exists("EFS_Nav_Menu_Options") &&
        !empty(self::$args["nav_menu_options"])
      ) {
      ...
      $params = [];
      if (
        class_exists("EFS_Profile_Options") &&
        !empty(self::$args["profile_options"])
      ) {
      ...
      $params = [];
      if (
        class_exists("EFS_Taxonomy_Options") &&
        !empty(self::$args["taxonomy_options"])
      ) {
      ...
      if (
        class_exists("EFS_Widget") &&
        class_exists("WP_Widget_Factory") &&
        !empty(self::$args["widget_options"])
      ) {
      ...
      $params = [];
      if (
        class_exists("EFS_Comment_Metabox") &&
        !empty(self::$args["comment_options"])
      ) {
      ...
      $params = [];
      if (
        class_exists("EFS_Shortcoder") &&
        !empty(self::$args["shortcode_options"])
      ) {
        ...
        if (
          class_exists("EFS_Shortcoder") &&
          !empty(self::$shortcode_instances)
        ) {
    ...
    public static function constants()
    {
      ...
      $dirname = str_replace(
        "//",
        "/",
        wp_normalize_path(dirname(dirname(self::$file)))
      );
      $theme_dir = str_replace(
        "//",
        "/",
        wp_normalize_path(get_parent_theme_file_path())
      );
    ...
    public static function includes()
    {
      ...
      if (!empty($fields)) {
        foreach ($fields as $field) {
          if (
            !class_exists("EFS_Field_" . $field) &&
            class_exists("EFS_Fields")
          ) {
            self::include_plugin_file(
              "fields/" . $field . "/" . $field . ".php"
            );
          }
        }
      }
    ...
    public static function add_admin_enqueue_scripts()
    {
      ...
      if (!empty(self::$args["admin_options"])) {
        foreach (self::$args["admin_options"] as $argument) {
          if (
            substr($wpscreen->id, -strlen($argument["menu_slug"])) ===
            $argument["menu_slug"]
          ) {
            self::$enqueue = true;
          }
      ...
      if (!empty(self::$shortcode_instances)) {
        foreach (self::$shortcode_instances as $argument) {
          if (
            ($argument["show_in_editor"] && $wpscreen->base === "post") ||
            $argument["show_in_custom"]
          ) {
            self::$enqueue = true;
          }
        }
      ...
      if (
        !empty(self::$args["widget_options"]) &&
        ($wpscreen->id === "widgets" || $wpscreen->id === "customize")
      ) {
        self::$enqueue = true;
      }
      ...
      if (
        !empty(self::$args["profile_options"]) &&
        ($wpscreen->id === "profile" || $wpscreen->id === "user-edit")
      ) {
        self::$enqueue = true;
      }
      ...
      if (apply_filters("csf_fa4", false)) {
        wp_enqueue_style(
          "csf-fa",
          "https://cdn.jsdelivr.net/npm/font-awesome@4.7.0/css/font-awesome" .
            $min .
            ".css",
          [],
          "4.7.0",
        ...
        wp_enqueue_style(
          "csf-fa5",
          "https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.3/css/all" .
            $min .
            ".css",
          [],
          "5.15.3",
        ...
        wp_enqueue_style(
          "csf-fa5-v4-shims",
          "https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.3/css/v4-shims" .
            $min .
            ".css",
          [],
          "5.15.3",
      ...
      wp_enqueue_style(
        "csf",
        self::include_plugin_url("assets/css/style" . $min . ".css"),
        [],
        self::$version,
        "all"
      );
      ...
      if (is_rtl()) {
        wp_enqueue_style(
          "csf-rtl",
          self::include_plugin_url("assets/css/style-rtl" . $min . ".css"),
          [],
          self::$version,
          "all"
        );
      ...
      wp_enqueue_script(
        "csf-plugins",
        self::include_plugin_url("assets/js/plugins" . $min . ".js"),
        [],
        self::$version,
        true
      );
      wp_enqueue_script(
        "csf",
        self::include_plugin_url("assets/js/main" . $min . ".js"),
        ["csf-plugins"],
        self::$version,
        true
      );
        ...
        foreach (self::$fields as $field) {
          if (!empty($field["type"])) {
            ...
            if (
              ...
              method_exists($classname, "enqueue")
            ) {
      ...
      if (!empty(self::$webfonts)) {
        if (!empty(self::$webfonts["enqueue"])) {
          ...
          foreach (self::$webfonts["enqueue"] as $family => $styles) {
            $fonts[] =
              $family . (!empty($styles) ? ":" . implode(",", $styles) : "");
          }
          ...
          wp_enqueue_style(
            ...
            esc_url(add_query_arg($query, "//fonts.googleapis.com/css")),
            [],
            null
          );
        }
        ...
        if (!empty(self::$webfonts["async"])) {
          ...
          wp_localize_script("csf-google-web-fonts", "WebFontConfig", [
    ...
    public static function field(
      ...
      if (!empty($field["_notice"])) {
        ...
        $field = [];
        $field["content"] =
          esc_html__("Oops! Not allowed.", "csf") .
          " <strong>(" .
          $field_type .
          ")</strong>";
        $field["type"] = "notice";
      ...
      if (!empty($field_type)) {
        ...
        echo '<div class="csf-field csf-field-' .
          $field_type .
          $is_pseudo .
          $class .
          $visible .
        ...
        if (!empty($field["fancy_title"])) {
          echo '<div class="csf-fancy-title">' .
            $field["fancy_title"] .
            "</div>";
        }
        ...
        if (!empty($field["title"])) {
          ...
          echo !empty($field["subtitle"])
            ? '<div class="csf-subtitle-text">' . $field["subtitle"] . "</div>"
            : "";
          echo "</div>";
        }
        ...
        $value =
          !isset($value) && isset($field["default"])
            ? $field["default"]
            : $value;
